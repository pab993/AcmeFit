...
<display:table name="customers" id="row" requestURI="${requestURI}"
	...
	<security:authorize access="hasAnyRole('TRAINER', 'NUTRITIONIST')">
		...
		<display:column title="${dietsHeader}">
			<a href="diet/listDiets.do?customerId=${row.id}"> <spring:message
					code="customer.diets" />
			</a>
		</display:column>
	</security:authorize>
	...
	<security:authorize access="hasAnyRole('TRAINER', 'NUTRITIONIST')">
		...
		<display:column title="${activitiesHeader}">
			<a href="activity/listActivities.do?customerId=${row.id}"> <spring:message
		...
		<security:authorize access="hasRole('ADMINISTRATOR')">
			<display:column title="${activitiesHeader}">
				<a href="invoice/administrator/create.do?customerId=${row.id}"> <spring:message
					code="customer.administrator.invoice" />
				</a>
			</display:column>
	</security:authorize>
